...
//DECLARE VARIABLES
	...
	var walkability = new Array (mapWidth+1,mapHeight+1); //array that holds wall/obstacle information	
	var openList = new Array (mapWidth*mapHeight+2); //1 dimensional array holding ID# of open list items
	var whichList = new Array (mapWidth+1,mapHeight+1);  //2 dimensional array used to record 
		//whether a cell is on the open list or on the closed list.
	var openX = new Array (mapWidth*mapHeight+2); //1d array stores the x location of an item on the open list
	var openY = new Array (mapWidth*mapHeight+2); //1d array stores the y location of an item on the open list
	var parentX = new Array (mapWidth+1,mapHeight+1); //2d array to store parent of each cell (x)
	var parentY = new Array (mapWidth+1,mapHeight+1); //2d array to store parent of each cell (y)
	var Fcost = new Array (mapWidth*mapHeight+2);	//1d array to store F cost of a cell on the open list
	var Gcost = new Array (mapWidth+1,mapHeight+1); 	//2d array to store G cost for each cell.
	var Hcost = new Array (mapWidth*mapHeight+2);	//1d array to store H cost of a cell on the open list		
	
	//Declare constants
	var onClosedList = 10; //openList variable	
	var notfinished = 0;
	var notStarted = 0;
	var found = 1;
	var nonexistent = 2;// pathStatus constants 
	var walkable = 0;
	var unwalkable = 1;// walkability array constants
...
//1.	Convert location data (in pixels) to coordinates in the walkability array.
	startX = Floor(unit\xLoc/tileSize);
	startY = Floor(unit\yLoc/tileSize);
	targetX = Floor(targetX/tileSize);
	targetY = Floor(targetY/tileSize);

//2.	Quick Path Checks: Under the some circumstances no path needs to
	//be generated ...

	//If starting location and target are in the same location...
	if (startX == targetX && startY == targetY && unit.pathLocation > 0){
		 return found
	}
	if (startX == targetX && startY == targetY && unit.pathLocation == 0){
		return nonexistent
...
//3.	Reset some variables that need to be cleared
	if (onClosedList > 1000000){ //occasionally redim whichList
		var whichList = new Array (mapWidth,mapHeight);
		onClosedList = 10;
	}
...
//6.	If the open list is not empty, take the first cell off of the list.
	...
	if (numberOfOpenListItems <> 0){
		...
		v = 1;	
		while(1){ //Repeat the following until the new item in slot #1 sinks to its proper spot in the heap.
			u = v;	
...
} //repeat until path is found or deemed nonexistent
